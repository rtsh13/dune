...
object MatrixGenerator {
  ...
  def generateSparseMatrix(
    ...
    println(
      s"Generating ${numRows}x${numCols} matrix with ${sparsity * 100}% sparsity..."
    )
  ...
  /** Generate a sparse vector and save to CSV Only stores non-zero entries
    */
  def generateSparseVector(
    ...
    println(
      s"Generating sparse vector of size $size with ${sparsity * 100}% sparsity..."
    )
  ...
  def main(args: Array[String]): Unit = {
    ...
    generateSparseMatrix(
      ...
      10000,
      0.99,
      "data/test/large_sparse_10kx10k.csv"
    )

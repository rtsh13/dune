...
def runEndToEndEvaluation(
  ...
  private def testIterativeAlgorithm(
    ...
    val iterations = 10
    println(s"Running PageRank-like algorithm for $iterations iterations")
    ...
    for (i <- 1 to iterations) {
      val result = FormatSpecificOps.cooSpMV(matrix.entries, vector)
      vector = result
      if (i % 2 == 0) {
        vector.count() // Force evaluation
...
private def testMatrixChain(
    ...
    val AB_COO = FormatSpecificOps.cooSpMMSparse(matrix.entries, matrix.entries)
    val ABC_COO = FormatSpecificOps.cooSpMVSparse(AB_COO, sparseVectorEntries)
    ABC_COO.count()
    ...
    val AB_CSR = FormatSpecificOps.csrSpMMSparse(csrMatrix.rows, csrMatrix.rows)
    val ABC_CSR = FormatSpecificOps.csrSpMVSparse(csrMatrix.rows, sparseVectorEntries)
    ABC_CSR.count()
    ...
    val AB_CSC = FormatSpecificOps.cscSpMMSparse(cscMatrix, cscMatrix)
    val ABC_CSC = FormatSpecificOps.cscSpMVSparse(cscMatrix, sparseVectorEntries)
    ABC_CSC.count()
  ...
  private def testRealWorldApplications(
    ...
    // Application 1: Graph Analysis
    println("\n  Application 1: Graph Analysis (Centrality)")
    val vectorRDD = vector.toIndexValueRDD

    val customStart1 = System.nanoTime()
    var result1 = FormatSpecificOps.cooSpMV(matrix.entries, vectorRDD)
    for (i <- 1 to 5) {
      result1 = FormatSpecificOps.cooSpMV(matrix.entries, result1)
    }
    result1.count()
    ...
    val similarity = FormatSpecificOps.cooSpMMSparse(matrix.entries, matrix.entries)
    val recommendations = FormatSpecificOps.cooSpMV(similarity, vectorRDD)

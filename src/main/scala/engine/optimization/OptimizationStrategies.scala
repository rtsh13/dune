...
object OptimizationStrategies {
  def cooSpMV_Baseline(
    ...
    val products = joined.map { case (col, ((row, mVal), vVal)) =>
      (row, mVal * vVal)
    }
  ...
  def cooSpMM_BlockPartitioned(
    ...
    val resultEntries = joinedBlocks.flatMap {
      case (innerBlock, ((rowBlockA, entriesA), (colBlockB, entriesB))) =>
        val localResults =
          scala.collection.mutable.HashMap[(Int, Int), Double]()
        for {
          entryA <- entriesA
          entryB <- entriesB
  ...
  def csrSpMVSparse_CoPartitioning(
    ...
    val vectorByIndex =
      vectorX.map(e => (e.index, e.value)).partitionBy(partitioner)
  ...
  def cscSpMV_CoPartitioning(
    ...
    val cscByCol =
      cscMatrix.map(col => (col.columnId, col)).partitionBy(partitioner)
    val vectorPartitioned = vectorX.partitionBy(partitioner)

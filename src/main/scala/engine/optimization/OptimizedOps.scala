...
/** Optimized versions of operations with:
  *   1. Smart partitioning to reduce shuffle 2. Caching strategies for reused
  *      data 3. Co-partitioning for joins
  */
object OptimizedOps {
  ...
  /** Optimized SpMV with intelligent partitioning
    *
    * Key optimizations:
    *   1. Partition both RDDs by join key (column index) 2. Use HashPartitioner
    *      for consistent hashing 3. Persist partitioned RDDs to avoid
    *      recomputation 4. Clean up after operation
    *
  ...
  /** Optimized SpMM with co-partitioning
    *
    * Key optimizations:
    *   1. Partition both matrices by inner dimension 2. Co-locate join keys to
    *      minimize shuffle 3. Use efficient storage levels 4. Repartition
    *      result for balanced output
    *
  ...
  /** Block-based matrix multiplication for very large matrices
    ...
    *   1. Better memory locality 2. Reduced shuffle volume 3. Improved cache
    *      utilization
    *
  ...
  def blockPartitionedSpMM(
    ...
    println(
      s"Block-partitioned SpMM complete. Result has ${result.count()} non-zeros."
    )
  ...
  /** Cache management for operations that reuse matrices
    *
    * Example: Computing A × B, A × C, A × D Matrix A should be cached and
    * reused
    */
  def cacheMatrix(
      matrix: RDD[COOEntry],
  ...
  /** Automatically determine optimal number of partitions based on matrix
    * properties
    *
    * Heuristic:
  ...
  def computeOptimalPartitions(
    ...
    // Clamp to min/max
    val optimalPartitions =
      math.max(minPartitions, math.min(maxPartitions, calculatedPartitions))

    println(s"Computed optimal partitions: $optimalPartitions")
    println(
      s"  (for $numEntries entries, target: $targetEntriesPerPartition per partition)"
    )

...
object MultiplicationOps {
  ...
  // Operation 1: Sparse Matrix × Dense Vector (SpMV)
  
  /**
   * Sparse Matrix × Dense Vector: y = A × x
   * 
   * Algorithm:
   * 1. Matrix A: (row, col, value) → rekey to (col, (row, value))
   * 2. Vector x: (index, value) - already keyed by index
   * 3. Join on column = index
   * 4. Multiply: matrix_value × vector_value
   * 5. Group by row and sum
   * 
   * Time Complexity: O(nnz(A) + nnz(x)) where nnz = non-zeros
   * Space Complexity: O(nnz(A)) distributed
   */
   ...
   * 
   * Similar to dense vector version but both operands are sparse
   * Only non-zero entries participate in join
   */
   ...
   * 
   * Algorithm:
   * For C[i,k] = Σ(j) A[i,j] × B[j,k]
   * 
   * 1. Matrix A: (i, j, vA) → rekey to (j, (i, vA))
   * 2. Matrix B: (j, k, vB) - already keyed by j
   * 3. Join on j (inner dimension)
   * 4. For each match: emit ((i, k), vA × vB)
   * 5. Sum all partial products for each (i, k)
   */
   ...
   * 
   * CSR format is optimized for row-wise operations
   * Each CSRRow computes its dot product with the vector independently
   * 
   * Algorithm:
   * 1. For each row in CSR matrix
   * 2. Join row's column indices with vector
  ...
  def csrMatrixDenseVector(
    ...
    println("Computing CSR Matrix × Dense Vector...")
    ...
    println(s"CSR SpMV complete. Result has ${result.count()} non-zero entries.")

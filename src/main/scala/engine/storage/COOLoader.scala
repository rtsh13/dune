...
object COOLoader {
  ...
  /** Loads a sparse matrix from CSV file into COO format Returns RDD - fully
    * distributed
    */
  ...
  /** Load a dense vector from CSV file Returns RDD
    *
  ...
  def calculateSparsity(
    ...
    println(
      f"Sparsity: ${sparsity * 100}%.2f%% ($numNonZeros non-zeros out of $totalElements)"
    )

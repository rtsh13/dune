...
case class SparseMatrixCOO(
  ...
  override def transpose: Matrix = {
    val transposedEntries =
      this.entries.map(e => COOEntry(e.col, e.row, e.value))
  ...
  def slice(
    ...
    val slicedEntries = this.entries
      .filter { entry =>
...
case class DenseMatrixRDD(
  ...
  override def multiply(matrix: Matrix): ResultMatrix = {
    matrix match {
      case denseMat: DenseMatrixRDD =>
        ...
        val matrixBByRow = denseMat.entries.map { case (r, c, v) =>
          (r, (c, v))
        }
  ...
  override def add(other: Matrix): ResultMatrix = {
    val otherEntries = other match {
      ...
      case csr: SparseMatrixCSR =>
        csr.toCOO.entries.map(e => (e.row, e.col, e.value))
    }

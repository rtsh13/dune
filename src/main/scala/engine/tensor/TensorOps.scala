...
object TensorOps {
  ...
  /** MTTKRP: Core operation in CP tensor decomposition
    ...
    *   1. For each non-zero tensor entry T[i,j,k,...] 2. Get corresponding rows
    *      from factor matrices 3. Compute element-wise product (Hadamard) 4.
    *      Accumulate result for output row
    */
  def mttkrp(
      tensor: SparseTensor,
  ...
  /** Load a 3D sparse tensor from CSV file Format: i,j,k,value
    */
  def loadSparseTensor3D(
    ...
    println(
      s"Loaded tensor: ${dimensions.mkString(" x ")} with $count non-zeros"
    )
